...
def process_args(args=None):
    ...
    args.heatmapHeight = (
        args.heatmapHeight
        if args.heatmapHeight > 3 and args.heatmapHeight <= 100
        else 10
    )

    if not matplotlib.colors.is_color_like(args.missingDataColor):
        exit(
            "The value {0}  for --missingDataColor is not valid".format(
                args.missingDataColor
            )
        )
...
def prepare_layout(
    ...
    if perGroup:
        ...
        height_ratio = np.array(
            [np.amax(np.diff(hm_matrix.group_boundaries))] * numrows
        )
    ...
    if wspace is not None and hspace is not None:
        if colorbar_position == "side":
            gridsSub = gridspec.GridSpecFromSubplotSpec(
                1, iterNum, subplot_spec=grids[0, :-1], wspace=wspace, hspace=hspace
            )
        else:
            gridsSub = gridspec.GridSpecFromSubplotSpec(
                1, iterNum, subplot_spec=grids[0, :], wspace=wspace, hspace=hspace
            )
...
def plotlyMatrix(
    ...
    if colorBarPosition != "side":
        sys.error.write(
            "Warning: It is not currently possible to have multiple colorbars with plotly!\n"
        )
    ...
    if showSummaryPlot:
        ...
        profileSideBuffer = 0.0
        profileWidth = 1.0 / nCols
        if nCols > 1:
    ...
    if showSummaryPlot:
        ...
        for i in range(nCols):
            ...
            for j in range(nRows):
                ...
                trace = plotly_single(mat["matrix"], averageType, colorList[j], label)[
                    0
                ]
    ...
    zMinLocal = np.inf
    zMaxLocal = -np.inf
    heatmapWidth = 1.0 / nCols
    heatmapSideBuffer = 0.0
    if nCols > 1:
        heatmapWidth = 0.9 / nCols
    ...
    for i in range(nCols):
        ...
        for j in range(nRows):
            ...
            yanchor = "y{}".format(yAxisN)
            yDomain = [
                heatmapHeight - fractionalHeights[j + 1],
                heatmapHeight - fractionalHeights[j],
            ]
            visible = False
            if i == 0:
                visible = True
    ...
    if showColorbar:
        ...
        dataHeatmap[-1]["colorbar"].update(
            len=heatmapHeight, y=0, yanchor="bottom", ypad=0.0
        )
    ...
    for trace in dataHeatmap:
        ...
        trace.update(
            zmin=zMinUse,
            zmax=zMaxUse,
            colorscale=convertCmap(cmap[0], vmin=zMinUse, vmax=zMaxUse),
        )
...
def plotMatrix(
    ...
    if (len(zMin) > 1) & (len(zMax) > 1):
        for index, value in enumerate(zMax):
            if value <= zMin[index]:
                sys.stderr.write(
                    "Warnirng: In bigwig {}, the given zmin ({}) is larger than "
    ...
    if perGroup:
        color_list = cmap_plot(
            np.arange(hm.matrix.get_num_samples()) / hm.matrix.get_num_samples()
        )
    else:
    ...
    for sample in range(hm.matrix.get_num_samples()):
        ...
        for group in range(numgroups):
            ...
            sub_matrix["matrix"] = np.clip(
                sub_matrix["matrix"], zMin[zmin_idx], zMax[zmax_idx]
            )
            img = ax.imshow(
                sub_matrix["matrix"],
                aspect="auto",
            ...
            if regions_length_in_bins[sample] is not None:
                ...
                ax.plot(
                    regions_length_in_bins[sample][group_idx],
            ...
            if perGroup and group == 0:
                ax.axes.set_ylabel(
                    sub_matrix["sample"], rotation=75, labelpad=0, fontsize=15
                )
            ...
            if linesAtTickMarks:
                ...
                xticks_heat = [
                    x + 0.5 for x in xticks_heat
                ]  # There's an offset of 0.5 compared to the profile plot
            ...
            if (perGroup and sample == numsamples - 1) or (
                ...
                ax.get_xaxis().set_tick_params(which="both", top=False, direction="out")
                ...
                if showColorbar and colorbar_position == "below":
                    ...
                    cbar = fig.colorbar(
                        img, cax=ax, orientation="horizontal", ticks=tick_locator
                    )
    ...
    if box_around_heatmaps:
        fig.get_layout_engine().set(
            wspace=0.10, hspace=0.025, rect=(0.04, 0, 0.96, 0.85)
        )
    else:
        #  When no box is plotted the space between heatmaps is reduced
        fig.get_layout_engine().set(
            wspace=0.05, hspace=0.01, rect=(0.04, 0, 0.96, 0.85)
        )

    plt.savefig(
        outFileName, bbox_inches="tight", pad_inches=0.1, dpi=dpi, format=image_format
    )
    plt.close()
    ...
    for label, ma in matrixDict.items():
        ...
        if group_lengths[i] > min_group_length:
            if len(to_merge):
                ...
                new_ma = np.concatenate([matrixDict[item] for item in to_merge], axis=0)
            else:
                new_label = label
        ...
        if args.sortUsingSamples is not None:
            for i in args.sortUsingSamples:
                if i > 0 and i <= hm.matrix.get_num_samples():
                    ...
                    exit(
                        "The value {0} for --sortSamples is not valid. Only values from 1 to {1} are allowed.".format(
                            args.sortUsingSamples, hm.matrix.get_num_samples()
                        )
                    )
            print("Samples used for ordering within each group: ", sortUsingSamples)

        hm.matrix.sort_groups(
            sort_using=args.sortUsing,
